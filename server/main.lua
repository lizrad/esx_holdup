local rob = false
local robbers = {}
local AccountName = Config.AccountName[Config.AccountIndex]
ESX = nil
        ...
        for i = 1, #xPlayers, 1 do
            ...
            if xPlayer.job.name == "police" then
                TriggerClientEvent(
                    ...
                    xPlayers[i],
                    _U("robbery_cancelled_at", Stores[currentStore].nameOfStore)
                )
        ...
        if Stores[currentStore] then
            ...
            if (os.time() - store.lastRobbed) < Config.TimerBeforeNewRob and store.lastRobbed ~= 0 then
                TriggerClientEvent(
                    "esx:showNotification",
                    _source,
                    _U("recently_robbed", Config.TimerBeforeNewRob - (os.time() - store.lastRobbed))
                )
                return
            end
                    ...
                    for i = 1, #xPlayers, 1 do
                        ...
                        if xPlayer.job.name == "police" then
                            TriggerClientEvent(
                                "esx:showNotification",
                                xPlayers[i],
                                _U("rob_in_prog", store.nameOfStore)
                            )
                            ...
                            if robbers[_source] then
                                ...
                                if xPlayer then
                                    ...
                                    if Config.GiveBlackMoney then
                                        xPlayer.addAccountMoney(AccountName, store.reward)
                                    else
                                    ...
                                    for i = 1, #xPlayers, 1 do
                                        ...
                                        if xPlayer.job.name == "police" then
                                            TriggerClientEvent(
                                                "esx:showNotification",
                                                xPlayers[i],
                                                _U("robbery_complete_at", store.nameOfStore)
                                            )
                                            TriggerClientEvent("esx_holdup:killBlip", xPlayers[i])
                                        end
                                    end
